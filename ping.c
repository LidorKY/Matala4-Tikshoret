...
int main(int args,char** dest)
{
    ...
    // inet_pton(AF_INET, DESTINATION_IP, &(dest_in.sin_addr.s_addr));
        ...
        int bytes_sent = sendto(sock, packet, ICMP_HDRLEN + datalen, 0, (struct sockaddr *)&dest_in, sizeof(dest_in));//sending the ping
        if (bytes_sent == -1)
        {
        ...
        //memcpy(reply, packet + ICMP_HDRLEN + IP4_HDRLEN, datalen); copieng the response to an array called reply.
        //printf("ICMP reply: %s \n", reply);
